...
`TaskCompletionService` が日本語のシステムプロンプトで Anthropic API を呼び出す。タスク作成・タイトル更新時は `call_combined` で提案文と優先度を1回の API 呼び出しで取得（タイムアウト4秒）。Ajax プレビューは `call_with_title`（保存なし）。API 障害時はフォールバックメッセージ・デフォルト優先度（medium）を返す。
