...
`TaskCompletionService` が日本語のシステムプロンプトで Anthropic API を呼び出す。タスク作成・タイトル更新時は `call_combined` で提案文と優先度を1回の API 呼び出しで取得（タイムアウト4秒）。Ajax プレビューは `call_with_title`（保存なし）。API 障害時は提案にフォールバックメッセージを設定。優先度は新規作成時は enum デフォルト（medium）のまま、更新時の再推論失敗は既存値を維持（medium へのリセットは行わない）。
...
- `test/services/` — TaskCompletionService（`call` / `call_priority` / `call_combined` / Anthropic API スタブ）
...
- 001-ai-priority-detect: AI 優先度自動判定を実装（`priority_manually_set` カラム追加、`TaskCompletionService#call_combined` で提案+優先度を1回のAPI呼び出しに統合）
